...
node("docker") {
	...
	if(params.USE_ROOT_NS)
	{
    	...
    	KUBE_NAMESPACE=NAMESPACE.replaceAll(/\./,"-")+"-"+TARGET_ENV
   	}
	...
	buildImageName = "${NAMESPACE}/${SERVICE_NAME}:latest"
	...
	branchName = (env.BRANCH_NAME ?: "master").replaceAll(/[^0-9a-zA-Z_]/, "-")
			...
			if ("${PHASE}" == "BUILD" || "${PHASE}" == "BUILD_DEPLOY" ) {
				...
				stage 'Package'
				sh 'mvn -X -s $MAVEN_SETTINGS -Ddummy.prop=$SONAR_PROPERTIES com.github.eirslett:frontend-maven-plugin:grunt -Dfrontend.grunt.arguments="docker-image docker-push"' 
